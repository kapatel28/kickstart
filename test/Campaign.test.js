...
beforeEach(async () => {
  ...
  await factory.methods.createCampaign('100').send({
   ...
   // leting web3 know the contract ; not deploying
...
it('allows manager to to make a payment request', async () => {

await campaign.methods.createRequest('Buy woods!','100',accounts[2])
.send({
  from: accounts[0],
  gas: '1000000'
});
  ...
  assert.equal('Buy woods!',request.description);
...
it('processes requests', async () => {
  ...
  balance = parseFloat(balance); // string to parseFloat
  console.log(balance);
  assert(balance > 102);
